...
- ✅ **Python-data-jobb i GitHub Actions** som skriver ferdig OSEBX-JSON — **«ukens vinnere/tapere» (W/W) + dekningsvakt** mot tomt dashboard. **Gjort 2026-06-05** (osebx `220172c` data/analyze + `1a8f132` UI; nytt `returnsWeekly` + `weekly_movers`, skjules stille i mellomfasen). **Datadrevet sektor-gruppering var allerede oppfylt** (`update_data.py` henter Yahoos `sector` + normaliserer via `SECTOR_NORM`). **«Avlaster Worker-en» N/A** — dashboardet gjør 0 runtime-kall til finance-proxy (leser kun committet JSON). **yfinance `Screener` forkastet** — yahooquery beholdt (bedre for `.OL`; Screener kan ikke enumerere OSEBX-konstituenter). *(yfinance, OpenBB som mer robust data-lag)*
- ✅ (Valgfritt, lett) **«Investor-persona»-vinkel** i LLM-narrativet («hva ville en verdiinvestor sett her?»). **Gjort 2026-06-05** (osebx `80c4f25`); verdiinvestor-linse som siste setning i `market_take` — edukasjonell, aldri anbefaling (prompt-regel 10, forbyr kjøp/selg/direktiv). *(FinceptTerminal — eneste lette idé derfra)*
